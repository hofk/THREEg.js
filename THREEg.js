...
function createCurved2Geometry( design, centerMorph, morphCount ) { // optional: centerMorph, morphCount 
	
	const g = this;   //  THREE.BufferGeometry() - geometry object from three.js
	
	g.connected = design[ 0 ][ 1 ][2] === 'x' ? false : true;
	g.symmetric = design[ 0 ][ 1 ][2] === 's' ? true : false;
	
	g.dsgn = [];
	
	g.centerMorph = centerMorph;
	g.morphCount = morphCount !== undefined ? morphCount : 24;	
		...
		for ( let i = 0; i < design.length; i ++ ) {
			...
			if ( i === 0 ) {
				...
				for ( let j = len - 2; j >= 4; j -- ) {
					...
					g.dsgn[ 0 ].push( 180 + 180 - design[ 0 ][ j ] );
	...
	g.rdefCount = g.dsgn[ 0 ].length - 3; // radial definition count
	
	g.radial = g.dsgn[ 0 ][ 2 ];		// radial max 
	g.vertical = g.dsgn[ 0 ][ 0 ];	// vertical max
	
	g.wTop = g.dsgn[ 0 ][ 1 ][0] !== 'x' ? true : false;
	g.flatTop = g.dsgn[ 0 ][ 1 ][0] === 'f' ? true : false;
	
	g.wBtm = g.dsgn[ 0 ][ 1 ][1] !==  'x' ? true : false;
	g.flatBtm = g.dsgn[ 0 ][ 1 ][1] === 'f' ? true : false;
	
	const angle = g.dsgn[ 0 ][ g.dsgn[ 0 ].length - 1 ];   
	const dAngle = angle / g.radial;	
	
	let angles = [ ]; // all radial angles in ° (g.radial + 1 many)
	g.phi = []; // closest angle to the design in radiant (g.rdefCount many)
	...
	for ( let i = 0; i < g.rdefCount; i ++ ) { // angle adaptation
		
		for ( let j = 0; j <= g.radial; j ++ ) {
		
			if ( Math.abs( g.dsgn[ 0 ][ i + 3 ] - angles[ j ] ) < dAngle / 2 ) {
				
				g.phi.push( angles[ j ] * Math.PI / 180 ); // with degree to radiant
	...
	g.buildtCurved2Geometry = buildtCurved2Geometry;
	g.buildtCurved2Geometry( );
	
}	
	
function buildtCurved2Geometry( ) {
	...
	for ( let i = 1; i < g.dsgn.length; i ++ ) {
		
		g.dsgnCenters.push( new THREE.Vector3( g.dsgn[ i ][ 0 ], g.dsgn[ i ][ 1 ], g.dsgn[ i ][ 2 ] ) );
		
	}
	
	g.cPoints = new THREE.CatmullRomCurve3( g.dsgnCenters, false ).getSpacedPoints( g.vertical );
	
	if ( g.centerMorph !== undefined ) {
		
		g.centerMrph = []; // vectors (center morph)  to create curve 
		g.cMorph = []; // g.morphCount many points of the curve (for each defined center)
		g.cPts = []; // array of array of morphed cPoints
		
		for ( let i = 0; i < g.dsgn.length - 1; i ++ ) {
			
			g.centerMrph.push( [] );
			
			g.centerMrph[ i ].push( g.dsgnCenters[ i ] );  // Vector3,  from design array
			
			for ( let j = 0; j < g.centerMorph.length; j ++ ) {
				
				const x = g.dsgn[ i + 1 ][ 0 ] + g.centerMorph[ j ][ i ][ 0 ];
				const y = g.dsgn[ i + 1 ][ 1 ] + g.centerMorph[ j ][ i ][ 1 ];
				const z = g.dsgn[ i + 1 ][ 2 ] + g.centerMorph[ j ][ i ][ 2 ];
		...
		for ( let i = 0; i < g.morphCount; i ++ ) {
			...
			for ( let j = 0; j < g.dsgn.length - 1; j ++ ) {
	...
	for ( let j = 0; j < g.rdefCount; j ++ ) {
		
		let V2 = [ ];
		
		for ( let i = 0; i < g.dsgnCenters.length; i ++ ) {
			
			V2.push( new THREE.Vector2( arg2[ i ], g.dsgn[ i + 1 ][ j + 3 ] ) );
	...
	*/
	...
	for ( let j = 0; j < g.rdefCount; j ++ ) {
		...
		for ( let i = 0; i < g.dsgnCenters.length; i ++ ) {
			...
			V3a.push( new THREE.Vector3( arg2[ i ], g.dsgn[ i + 1 ][ j + 3 ], 0 ) );
	...
	for ( let i = 0; i <= g.vertical; i ++ ) {
		...
		for ( let j = 0 ; j < g.rdefCount; j ++  ) {
			...
			//V3.push( new THREE.Vector3( -Math.cos( g.phi[ j ] ) * pr2[ j ][ i ].y,  0,  Math.sin(  g.phi[ j ] ) * pr2[ j ][ i ].y ) );
			V3.push( new THREE.Vector3( -Math.cos( g.phi[ j ] ) * pr3[ j ][ i ].y,  0,  Math.sin(  g.phi[ j ] ) * pr3[ j ][ i ].y ) );
			
		}
		
		g.pts2D.push( new THREE.CatmullRomCurve3( V3, g.connected ).getSpacedPoints( g.radial ) );
		
	}
	
	BasicGeometry( g.radial, g.vertical, g.wTop, g.wBtm ); // create a rudimentary geometry
	...
	if ( g.connected ) { // calculate new normals at mantle seam
	...
	if ( g.wTop && !g.flatTop ) { // calculate new normals at top seam
	...
	if ( g.wBtm && !g.flatBtm ) { // calculate new normals at bottom seam
		
		for( let j = 0; j <= g.radial ; j ++ ) {
			
			const offs = g.wTop ? + g.radial + 2 : 0;
	...
	function BasicGeometry( radialSegments, heightSegments, withTop, withBottom ) {
		...
		for ( let y = 0; y <= heightSegments; y ++ ) {
			...
			if ( g.symmetric ) { // texture mirror image
		...
		if ( g.wTop ) generateCap( true );
		if ( g.wBtm ) generateCap( false );
...
exports.createCurved2Geometry = createCurved2Geometry;
exports.buildtCurved2Geometry = buildtCurved2Geometry;
